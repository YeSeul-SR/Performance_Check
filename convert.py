    ...
    try:
        ...
        if len(cpu_list) == len(df):
            ...
            if cpu_list and gpu_list:
                ...
                new_df = pandas.DataFrame(data=[data], index=[first_day], columns=["CPU Temperature", "GPU Temperature"])
                new_df.to_csv("./data/temperature.csv")
                for i in range(1, len(date_list)):
                    index = date_list[i]
                    new_df.loc[index] = [cpu_list[i], gpu_list[i]]
                    new_df.to_csv("./data/temperature.csv")
                print("saved the CPU temperature and GPU temperature in temperature.csv file")
    except:
